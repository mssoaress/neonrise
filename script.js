...
(function() {
  ...
  if (wrap) {
    wrap.classList.remove("reveal", "reveal-left", "reveal-right");
...
(function() {
  ...
  wrap.addEventListener("touchmove", e => {
    const dx = ts - e.touches[0].clientX;
    wrap.scrollLeft = sl + dx;
  }, { passive: true });

  // Mouse drag (desktop)
  let down = false, mx = 0, msl = 0;
  wrap.addEventListener("mousedown", e => {
    down = true; mx = e.clientX; msl = wrap.scrollLeft;
    wrap.style.cursor = "grabbing";
  });
  wrap.addEventListener("mouseup",    () => { down = false; wrap.style.cursor = ""; });
  wrap.addEventListener("mouseleave", () => { down = false; wrap.style.cursor = ""; });
  wrap.addEventListener("mousemove",  e => {
    if (!down) return;
    e.preventDefault();
    wrap.scrollLeft = msl - (e.clientX - mx);
  });
})();
